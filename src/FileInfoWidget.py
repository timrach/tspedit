...
class FileInfoWidget(SidebarWidget):
    ...
    def __init__(self, parent, datacontroller, **options):
        ...
        self._datacontroller.register_data('fileinfo', {'filename': None, 'comment': None})
    ...
    def _setup_gui(self):
        ...
        # STARTNODE LABEL
        self._startnode_label = tk.Label(self._sub_frame, text="Startnodes: []")
    ...
    def on_text_change(self):
        ...
        comment = self._comment_text.get(0.0, tk.END).replace('\n', '').strip(' \t\n\r')
        self._datacontroller.commit_change('fileinfo', {'filename': filename, 'comment': comment})

    def data_update(self, key, data):
        """ Handles updates for registered data"""
        if key is 'startnode':
            if data:
                res = [node.nid for node in data if node.start]
                self._startnode_label.config(text="Startnodes: " + str(res))
        elif key is 'nodes':
            self._dimension_label.config(text="Dimension: " + str(len(data)))
            self._groups_label.config(text="Groups: " + str(len(tspio.get_groups(data))))
        elif key is 'fileinfo':
            self._filename_label.config(text="Filename: " + str(data['filename']))
